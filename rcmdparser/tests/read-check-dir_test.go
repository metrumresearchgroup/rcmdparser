...
func TestReadCheckDir(t *testing.T) {
	...
	var cdtests = []struct {
		in       string
		expected rcmdparser.CheckOutputInfo
	}{
		{
			"noTestThat",
			rcmdparser.CheckOutputInfo{
				rcmdparser.TestInfo{true, false, nil},
				[]byte("log\n"),
				[]byte("install\n"),
			},
		},
		{
			"WithTestThat",
			rcmdparser.CheckOutputInfo{
				rcmdparser.TestInfo{true, true, []byte("tests\n")},
				[]byte("log\n"),
				[]byte("install\n"),
			},
		},
		{
			"FailedTest",
			rcmdparser.CheckOutputInfo{
				rcmdparser.TestInfo{true, true, []byte("failed-tests\n")},
				[]byte("log\n"),
				[]byte("install\n"),
			},
		},
	...
	for _, tt := range cdtests {
		actual, _ := rcmdparser.ParseCheckDir(testFS, tt.in)
