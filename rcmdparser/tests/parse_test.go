...
func TestParseTestLog(t *testing.T) {
	...
	for _, tt := range pttests {
		actual := rcmdparser.ParseTestLog(tt.in)
...
Complete output:
  ...
  Execution halted`,
		...
		Environment: rcmdparser.EnvirnomentInformation{},
		Notes:       []string{},
		Warnings: []string{
...
'library' or 'require' call not declared from: ‘testdoc’`,
	...
	var actual = rcmdparser.ParseCheckLog([]byte(inputLog))
	g := goblin.Goblin(t)
